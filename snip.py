...
def execute_code(lines, indices_tuple, args):
    with open("temp.sh", "w") as out_file:
        out_file.write(interpreters[args.type])
...
def main():
    ...
    parser.add_argument(
        "-t",
        "--type",
        help="interpreter [bash, python3, python2]",
        type=str,
        default="bash",
    )
    ...
    # Execute snippet
        ...
        if args.snippet.isnumeric():
            ...
            execute_code(stdout_split, snippet_to_run, args)
        else:
            for counter, snippet in enumerate(snippet_list):
                start, _ = snippet[0], snippet[1]
                first_line = stdout_split[start]
                line_split = first_line.split(" ")
                if args.snippet == line_split[3]:
                    execute_code(stdout_split, snippet, args)
                    break
